...
{
	$imaxdbhost = "localhost";
	$imaxdbuser = "relyon_newuser";
	$imaxdbpwd = "hotelleela28";
	$imaxdbname = "relyon_imax";
	
	$connection = mysqli_connect($imaxdbhost, $imaxdbuser, $imaxdbpwd) or die("Cannot connect to Mysql server host");
	mysqli_select_db($imaxdbname) or die("Cannot connect to database");
	$result = mysqli_query($query) or die(mysqli_error());
	mysqli_close($connection);
	return $result;
}
...
function sendsms($servicename, $tonumber, $smstext, $senddate, $sendtime)
{
	...
	else
	{
		...
		$query = "insert into `relyon_userlogin2`.`smslogs`(servicename, tonumber, smstext, senddate, sendtime)values('".$servicename."', '".$tonumber."', '".$smstext."', '".$senddate."', '".$sendtime."')";
...
{
	if ((!preg_match('/^9\d{9}$/', $tonumber)) && (!preg_match('/^919\d{9}$/', $tonumber)))
		return false;
	else
		return true;
}
